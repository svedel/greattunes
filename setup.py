...
code = 1
try:
    code = subprocess.call(['pip', 'install', 'torch===1.6.0+cpu', 'torchvision===0.7.0+cpu', '-f', 'https://download.pytorch.org/whl/torch_stable.html'])
    if code != 0:
        raise Exception('Torch and torchvision installation failed!')
except:
    try:
        code = subprocess.call(['pip3', 'install', 'torch===1.6.0+cpu', 'torchvision===0.7.0+cpu', '-f', 'https://download.pytorch.org/whl/torch_stable.html'])
        if code != 0:
            raise Exception('Torch and torchvision installation failed!')
    except:

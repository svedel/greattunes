...
class Initializers:
    ...
    def __initialize_from_covars(self, covars):
        """
        initialize covars for modeing. Currently only extract starting guess for each covariate, ASSUME each variable is continuous.
        TODO:
        ...
        return torch.tensor(guesses, device=self.device, dtype=self.dtype), torch.tensor([lower_bounds, upper_bounds],
                                                                                         device=self.device,
                                                                                         dtype=self.dtype)
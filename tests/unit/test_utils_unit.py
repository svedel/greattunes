import pytest
import torch
from greattunes.utils import __get_covars_from_kwargs, DataSamplers
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [torch.tensor([1.56, 12.8], dtype=torch.double), "greattunes.utils.__get_covars_from_kwargs: dimension mismatch in provided 'covars'. Was expecting torch tensor of size (1,<num_covariates>) but received one of size (2)."],
    ]
)
